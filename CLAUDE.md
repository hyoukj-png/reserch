...
**구성:** 에이전트 팀 모드. 분석가 3종(tech/design/content) 병렬 → benchmark-synthesizer 통합 → qa-validator 정합성 검증. 에이전트 정의는 `.claude/agents/`, 스킬은 `.claude/skills/`에서 관리. 산출물 7종: `05-components.md`, `07-performance-a11y.md`, `08-renewal-insights.md`, `BENCHMARK_RECIPE.md`, `MASTER_REPLICATION_PROMPT.md`, **`FRD.md`(리뉴얼 기능 명세서 — 구현팀 단일 정답 소스, As-Is ★실측 vs To-Be 결정 구분, 형식은 `skills/benchmark-recipe/references/frd-format.md`)**, `COMPLETION_REPORT.md`. `09-runtime-interactions.md`가 있으면 클릭 후 동적 상태의 1차 근거로 반드시 포함하며, FRD의 ★실측 표기는 09에 근거가 있을 때만 허용된다.
